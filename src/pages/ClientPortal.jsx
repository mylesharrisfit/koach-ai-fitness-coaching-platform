...
import {
  Home, Dumbbell, Salad, BarChart2, MessageSquare, LogOut,
  ChevronRight, CheckSquare, ArrowLeft, Send
} from 'lucide-react';
...
function PortalNutrition({ user }) {
  const { data: clients = [] } = useQuery({
    queryKey: ['portal-client-nt', user?.email],
    queryFn: () => base44.entities.Client.filter({ email: user.email }, '-created_date', 1),
    enabled: !!user?.email,
  });
  const myClient = clients[0];
  const { data: plans = [] } = useQuery({
    queryKey: ['portal-nutrition-nt', myClient?.assigned_nutrition_id],
  ...
  return (
    <div className="px-5 pt-12 pb-28 space-y-5">
      <div className="mb-2">
        ...
        <h1 className="text-white text-xl font-bold mt-0.5">{plan?.title || 'Your Nutrition Plan'}</h1>
      </div>
      {!plan ? (
        <div className="p-6 rounded-2xl text-center" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              { label: 'Calories', val: plan.calories, color: '#F59E0B' },
              { label: 'Protein', val: plan.protein_g ? `${plan.protein_g}g` : null, color: '#3B82F6' },
              { label: 'Carbs', val: plan.carbs_g ? `${plan.carbs_g}g` : null, color: '#10B981' },
              { label: 'Fats', val: plan.fats_g ? `${plan.fats_g}g` : null, color: '#EF4444' },
            ].filter(x => x.val).map(x => (
              <div key={x.label} className="p-3 rounded-2xl text-center" style={{ background: `${x.color}10`, border: `1px solid ${x.color}20` }}>
          ...
          {(plan.meals || []).map((m, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                {m.time && <p className="text-white/30 text-[10px]">{m.time}</p>}
              </div>
              {(m.foods || []).slice(0, 3).map((f, j) => (
                <p key={j} className="text-white/40 text-xs py-0.5">{f.food_name} · {f.portion}</p>
              ))}
            </motion.div>
          ))}
        </div>
