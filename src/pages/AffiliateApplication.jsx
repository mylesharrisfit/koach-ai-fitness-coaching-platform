  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-2xl mx-auto px-4 pb-16">
        <h2 className="text-2xl font-black text-slate-900 mb-6">Apply to Join</h2>
        <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-2xl border border-slate-200 p-8">

          {/* Name & Email (pre-filled) */}
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-bold text-slate-900 mb-2">Full Name</label>
              <input type="text" value={user?.full_name || ''} disabled className="w-full px-4 py-2 rounded-lg bg-slate-100 text-slate-500" />
            </div>
            <div>
              <label className="block text-sm font-bold text-slate-900 mb-2">Email</label>
              <input type="email" value={user?.email || ''} disabled className="w-full px-4 py-2 rounded-lg bg-slate-100 text-slate-500" />
            </div>
          </div>

          {/* Website/URL */}
          <div>
            <label className="block text-sm font-bold text-slate-900 mb-2">Website or Social URL *</label>
            <input
              type="url"
              placeholder="https://..."
              value={formData.website_url}
              onChange={(e) => setFormData({ ...formData, website_url: e.target.value })}
              className="w-full px-4 py-2 rounded-lg border border-slate-200 text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-400"
              required
            />
          </div>
          ...
          <div>
            <label className="block text-sm font-bold text-slate-900 mb-2">Your Platforms *</label>
          ...
          {/* Audience size */}
          <div>
            <label className="block text-sm font-bold text-slate-900 mb-2">Audience Size *</label>
            <select
          ...
          {/* Promotion plan */}
          <div>
            <label className="block text-sm font-bold text-slate-900 mb-2">How will you promote KOACH AI? *</label>
            <textarea
          ...
          {/* Content output */}
          <div>
            <label className="block text-sm font-bold text-slate-900 mb-2">Monthly Content Output *</label>
            <select
          ...
          {/* Used KOACH AI */}
          <div>
            <label className="block text-sm font-bold text-slate-900 mb-3">Have you used KOACH AI?</label>
          ...
          <p className="text-center text-xs text-slate-500">⏱ Applications reviewed within 48 hours</p>
        </form>
      </div>
    </div>
  );
