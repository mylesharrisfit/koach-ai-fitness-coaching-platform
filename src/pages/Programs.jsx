    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[#374151] mt-0.5">{programs.length} program{programs.length !== 1 ? 's' : ''}</p>
        </div>
        <Button
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-[#E7EAF3] shadow-sm">
          <div className="w-14 h-14 rounded-2xl bg-[#F6F7FB] border border-[#E7EAF3] flex items-center justify-center mx-auto mb-4">
            <Dumbbell className="w-6 h-6 text-[#374151]" />
          </div>
          <p className="font-semibold text-[#1F2A44]">No programs yet</p>
          <p className="text-sm text-[#374151] mt-1 mb-5">Create your first workout program to get started.</p>
          ...
          {programs.map(program => (
            ...
            >
              <div className="p-5">
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0 pr-2">
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-2.5">
                      <span className={cn('text-[11px] font-semibold px-2 py-0.5 rounded-lg capitalize', difficultyColors[program.difficulty] || 'bg-[#F6F7FB] text-[#374151] border border-[#E7EAF3]')}>
                        {program.difficulty}
                      </span>
                      {program.category && (
                        <span className="text-[11px] font-medium px-2 py-0.5 rounded-lg bg-[#F6F7FB] text-[#374151] border border-[#E7EAF3] capitalize">
                    ...
                    {program.description && (
                      <p className="text-sm text-[#374151] mt-1 line-clamp-2 leading-relaxed">{program.description}</p>
                    )}
                  </div>
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-8 w-8 flex-shrink-0 opacity-100 md:opacity-0 md:group-hover:opacity-100 text-[#374151] hover:text-[#1F2A44] hover:bg-[#F6F7FB]"
                    ...
                    <DropdownMenuContent align="end" onClick={e => e.stopPropagation()}>
                      ...
                      {canUseTemplates ? (
                        ...
                        <DropdownMenuItem onClick={() => openUpgradeModal('program_templates')} className="text-[#374151] gap-2">
                ...
                {/* Stats row */}
                <div className="flex items-center gap-4 text-xs text-[#374151] pt-3 border-t border-[#E7EAF3]">
