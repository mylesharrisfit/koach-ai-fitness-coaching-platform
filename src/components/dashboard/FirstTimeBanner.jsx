    ...
    >
      ...
      <div className="px-5 py-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          <button onClick={onDismiss} className="p-1.5 rounded-lg flex-shrink-0 hover:bg-white/5 transition-colors" style={{ color: 'rgb(var(--muted-foreground))' }}>
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-xs font-bold flex-shrink-0" style={{ color: 'rgb(var(--primary))' }}>{done}/{total}</span>
        </div>
          ...
          {CHECKLIST.map((item) => {
            ...
            return (
              <div key={item.id} className="flex items-center gap-3 rounded-xl px-3 py-2.5 group transition-all"
                ...
                <Icon className="w-3.5 h-3.5 flex-shrink-0" style={{ color: isDone ? 'rgb(var(--success))' : 'rgb(var(--primary))' }} />
                <span className="text-xs font-medium flex-1" style={{ color: isDone ? 'rgb(var(--success))' : 'rgb(var(--muted-foreground))', textDecoration: isDone ? 'line-through' : 'none' }}>
                  {item.label}
                </span>
                {!isDone && (
                  <Link to={item.path} className="opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                    <ArrowRight className="w-3 h-3" style={{ color: 'rgb(var(--primary))' }} />
                  </Link>
                )}
              </div>
            );
