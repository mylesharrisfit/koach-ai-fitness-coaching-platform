...
export default function ProgressStatsRow({ data, metric }) {
  ...
  const changeColor = change === 0 ? '#6B7280'
    ...
    <div className="grid grid-cols-4 gap-3">
      {stats.map(s => (
        <div key={s.label} className="bg-white border border-[#E5E7EB] rounded-xl p-4">
          <p className="text-[10px] font-semibold text-[#9CA3AF] uppercase tracking-wider mb-1">{s.label}</p>
          <div className="flex items-center gap-1.5">
            {s.Icon && <s.Icon className="w-4 h-4 flex-shrink-0" style={{ color: s.color }} />}
            <p className="text-xl font-bold text-[#111827]" style={s.color ? { color: s.color } : {}}>{s.value}</p>
          </div>
