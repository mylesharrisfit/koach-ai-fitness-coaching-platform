...
function Sparkline({ data, lowerIsBetter }) {
  if (data.length < 2) return <div className="h-8 text-[10px] text-[#9CA3AF] flex items-center">—</div>;
  const trend = data[data.length - 1].v - data[0].v;
  const improving = lowerIsBetter ? trend < 0 : trend > 0;
  const color = improving ? '#10B981' : trend === 0 ? '#9CA3AF' : '#EF4444';
  return (
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#E5E7EB]">
          <h3 className="text-sm font-semibold text-[#111827]">Body Measurements</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-0">
          {MEASUREMENTS.map((m) => {
            ...
            const changeColor = change === null ? '#9CA3AF' : change === 0 ? '#9CA3AF' : improving ? '#10B981' : '#EF4444';
            return (
              <div key={m.key} className="px-5 py-4 border-b border-r border-[#F3F4F6]">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-semibold text-[#374151]">{m.label}</p>
                    <p className="text-xl font-bold text-[#111827] mt-0.5">{current ? `${current}"` : '—'}</p>
                  </div>
                  <Sparkline data={data} lowerIsBetter={m.lowerIsBetter} />
                </div>
                <div className="flex gap-3 mt-2 text-[10px]">
                  <span className="text-[#9CA3AF]">Start: <span className="font-medium text-[#374151]">{start ? `${start}"` : '—'}</span></span>
                  {change !== null && (
      ...
      {historyRows.length > 0 && (
        <div className="bg-white border border-[#E5E7EB] rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-[#E5E7EB]">
            <h3 className="text-sm font-semibold text-[#111827]">Measurement History</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead className="bg-[#F9FAFB]">
                <tr>
                  <th className="px-4 py-2 text-left font-semibold text-[#6B7280]">Date</th>
                  {MEASUREMENTS.map(m => (
                    <th key={m.key} className="px-4 py-2 text-left font-semibold text-[#6B7280]">{m.label} (in)</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {historyRows.map(({ ci, measurements }, i) => (
                  <tr key={i} className="border-t border-[#F3F4F6] hover:bg-[#F9FAFB]">
                    <td className="px-4 py-2 text-[#374151]">{format(parseISO(ci.date), 'MMM d, yyyy')}</td>
                    {MEASUREMENTS.map(m => (
                      <td key={m.key} className="px-4 py-2 text-[#374151]">{measurements?.[m.key] ?? '—'}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
      ...
      {showModal && (
        <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-semibold text-[#111827]">Log Measurements</h3>
              <button onClick={() => setShowModal(false)}><X className="w-4 h-4 text-[#6B7280]" /></button>
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {MEASUREMENTS.map(m => (
                <div key={m.key}>
                  <label className="block text-xs text-[#6B7280] mb-1">{m.label} (inches)</label>
            ...
            <div className="flex gap-2 justify-end">
              <button onClick={() => setShowModal(false)} className="px-4 py-2 text-xs rounded-lg border border-[#E5E7EB] text-[#374151]">Cancel</button>
