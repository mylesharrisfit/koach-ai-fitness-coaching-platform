...
export default function Sidebar({ user, onUpgrade }) {
  ...
  return (
    <aside className={cn(
      "fixed left-0 top-0 h-screen bg-white border-r border-[#E7EAF3] z-50 flex-col transition-all duration-300 hidden md:flex",
      collapsed ? "w-[68px]" : "w-[240px]"
    )}>
      {/* Logo */}
      <div className={cn(
        "h-[64px] flex items-center border-b border-[#E7EAF3] flex-shrink-0",
        ...
        {!collapsed && (
          <>
            <span className="flex-1 font-heading font-bold text-[15px] text-[#1F2A44]">FitForge</span>
            <NotificationBell />
          </>
        )}
      </div>
      ...
      {/* Bottom */}
      <div className="p-3 border-t border-[#E7EAF3] space-y-0.5">
        {!collapsed && (
