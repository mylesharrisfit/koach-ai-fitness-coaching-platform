    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <p className="text-sm text-muted-foreground">Assign <span className="font-medium text-foreground">"{program?.title}"</span> to active clients instantly</p>
        </DialogHeader>
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto mt-1">
          {filtered.length === 0 ? (
            <p className="text-center text-sm text-muted-foreground py-6">No active clients found</p>
          ) : filtered.map(client => (
            <button
              key={client.id}
              type="button"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">{client.goal?.replace('_', ' ')}</p>
              </div>
              {selected.includes(client.id) && (
