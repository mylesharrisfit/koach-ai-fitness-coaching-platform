...
export function SelectionCard({ label, description, icon: Icon, selected, onClick, size = 'md' }) {
  return (
    <motion.button
      ...
      style={{
        background: selected ? 'rgb(var(--primary) / 0.08)' : '#161616',
        border: selected ? '1px solid rgb(var(--primary) / 0.5)' : '1px solid rgba(255,255,255,0.06)',
        boxShadow: selected ? '0 0 24px rgb(var(--primary) / 0.12)' : 'none',
      }}
    >
      {selected && (
        <div
      ...
      <div className="relative z-10 flex items-start gap-4">
        {Icon && (
          <div
            ...
            style={{
              background: selected ? 'rgb(var(--primary) / 0.15)' : 'rgba(255,255,255,0.04)',
            }}
          >
  ...
  return (
    <motion.button
      ...
      style={{
        ...
        border: selected ? '1px solid rgb(var(--primary) / 0.45)' : '1px solid rgba(255,255,255,0.06)',
        color: selected ? 'rgb(var(--card))' : '#7A7A7A',
        boxShadow: selected ? '0 0 16px rgb(var(--primary) / 0.15)' : 'none',
      }}
    >
      {label}
    </motion.button>
