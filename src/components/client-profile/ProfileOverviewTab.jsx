  ...
  return (
    <div className={cn('bg-card rounded-2xl border border-border overflow-hidden', className)}>
      <div className="flex items-center gap-2 px-4 pt-4 pb-3 border-b border-muted">
        {Icon && (
          <div className={cn('w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0', iconClass || 'bg-muted')}>
...
function MetricRow({ label, value, sub, bar, barColor, icon: Icon }) {
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-muted last:border-0">
      ...
      <div className="flex-1 min-w-0">
        {bar !== undefined ? (
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-foreground w-8 text-right tabular-nums">{bar}%</span>
          </div>
        ) : (
...
export default function ProfileOverviewTab({ client, checkIns, score }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {(alerts.length > 0 || pendingCount > 0) && (
        <div className="bg-card rounded-2xl border border-border overflow-hidden">
          ...
          <div className="p-4 space-y-2">
            {pendingCount > 0 && (
              <div className="flex items-center gap-2.5 bg-accent border border-accent rounded-xl px-3 py-2.5">
                <ClipboardCheck className="w-3.5 h-3.5 text-primary flex-shrink-0" />
                <span className="text-xs font-medium text-foreground flex-1">{pendingCount} check-in{pendingCount > 1 ? 's' : ''} awaiting review</span>
              </div>
            )}
            {alerts.map((a, i) => (
              <div key={i} className={cn(
                'flex items-center gap-2.5 rounded-xl px-3 py-2.5 border text-xs font-medium',
                a.type === 'warning' ? 'bg-warning/10 border-warning text-warning' : 'bg-accent border-accent text-primary'
              )}>
                <AlertTriangle className={cn('w-3.5 h-3.5 flex-shrink-0', a.type === 'warning' ? 'text-warning' : 'text-primary')} />
                {a.text}
              </div>
            ))}
          </div>
      ...
      {/* ── Quick snapshot ── */}
      <SectionCard title="Snapshot" icon={Activity} iconClass="bg-accent">
        <div className="grid grid-cols-2 gap-3 mb-1">
          {/* Adherence big */}
          <div className="bg-muted rounded-xl p-4 text-center col-span-1">
            ...
            <p className="text-[10px] text-muted-foreground mt-0.5">Adherence Score</p>
          </div>
          {/* Goal */}
          <div className="bg-muted rounded-xl p-4 text-center col-span-1">
            <Target className="w-4 h-4 mx-auto mb-1 text-muted-foreground" />
            <p className="text-sm font-bold text-foreground leading-snug">{goalLabels[client.goal] || 'General Fitness'}</p>
        ...
        {(latestWeight || client.target_weight) && (
          <div className="mt-3 border border-border rounded-xl p-3">
            ...
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              <span>Current: <strong className="text-foreground">{latestWeight || '—'} lbs</strong></span>
              {client.target_weight && <span>Target: <strong className="text-foreground">{client.target_weight} lbs</strong></span>}
            </div>
            {latestWeight && client.target_weight && (
      ...
      {lastCI && (
        <SectionCard title="Latest Check-in" icon={ClipboardCheck} iconClass="bg-success/10">
          <p className="text-[10px] text-muted-foreground mb-3">{format(new Date(lastCI.date), 'MMMM d, yyyy')}</p>

          <div className="grid grid-cols-3 gap-2 mb-3">
            {lastCI.weight && (
              <div className="bg-muted rounded-xl p-2.5 text-center">
                <Scale className="w-3.5 h-3.5 mx-auto mb-1 text-muted-foreground" />
                <p className="text-sm font-bold text-foreground tabular-nums">{lastCI.weight}</p>
                <p className="text-[9px] text-muted-foreground">lbs</p>
              </div>
            )}
            {lastCI.sleep_hours && (
              <div className="bg-muted rounded-xl p-2.5 text-center">
                <Moon className="w-3.5 h-3.5 mx-auto mb-1 text-primary" />
                <p className="text-sm font-bold text-foreground tabular-nums">{lastCI.sleep_hours}</p>
                <p className="text-[9px] text-muted-foreground">hrs sleep</p>
              </div>
            )}
            {lastCI.energy_level && (
              <div className="bg-muted rounded-xl p-2.5 text-center">
                <Zap className="w-3.5 h-3.5 mx-auto mb-1 text-warning" />
                <p className="text-sm font-bold text-foreground tabular-nums">{lastCI.energy_level}<span className="text-[9px] text-muted-foreground">/10</span></p>
          ...
          {(lastCI.compliance_training != null || lastCI.compliance_nutrition != null) && (
            <div className="space-y-2">
              ...
              {lastCI.compliance_nutrition != null && (
                <MetricRow label="Nutrition" bar={lastCI.compliance_nutrition} barColor="bg-success" />
              )}
            </div>
          )}
      ...
      {/* ── Client details ── */}
      <SectionCard title="Details" icon={Mail} iconClass="bg-muted">
      ...
      {client.tags?.length > 0 && (
        <SectionCard title="Tags" icon={Tag} iconClass="bg-muted">
          <div className="flex flex-wrap gap-1.5">
            {client.tags.map(tag => (
              <span key={tag} className="text-xs bg-accent/10 text-primary border border-accent rounded-lg px-2.5 py-1 font-medium">
                #{tag}
              </span>
            ))}
          </div>
      ...
      {/* ── Coach notes ── */}
      {client.notes && (
        <SectionCard title="Coach Notes" icon={ClipboardCheck} iconClass="bg-muted">
          <p className="text-sm text-foreground leading-relaxed whitespace-pre-wrap">{client.notes}</p>
        </SectionCard>
      )}
    </div>
  );
