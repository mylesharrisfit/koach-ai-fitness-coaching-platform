...
import { Dumbbell, Salad, Users, Package, Edit, Trash2, Copy, Check, EyeOff, UserPlus } from 'lucide-react';
...
const CATEGORY_STYLES = {
  ...
  bundle:   { icon: Package,   gradient: 'from-[#F5F3FF] to-[#EDE9FE]', badge: 'bg-[#F5F3FF] text-[#7C3AED]' },
};
...
export default function ProductDetailSheet({ listing, clients = [], open, onClose, onEdit, onDelete }) {
  ...
  const copyLink = () => {
    navigator.clipboard.writeText(`${window.location.origin}/store/${listing.id}`);
    setCopied(true);
      ...
      <SheetContent side="right" className="w-full max-w-md p-0 overflow-y-auto">
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="bg-[#F9FAFB] rounded-xl p-3 text-center border border-[#E5E7EB]">
              <p className="text-lg font-bold text-[#111827]">{listing.rating ? `★ ${listing.rating}` : '—'}</p>
              <p className="text-[10px] text-[#9CA3AF] mt-0.5">Rating</p>
            </div>
          </div>
          ...
          {/* Share link */}
          <button onClick={copyLink} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 border border-[#E5E7EB] rounded-xl text-sm font-semibold text-[#374151] hover:bg-[#F9FAFB] transition-colors">
          ...
          <div className="flex gap-2 pt-2 border-t border-[#E5E7EB]">
            ...
            <Button variant="outline" className="flex-1 text-red-500 border-red-200 hover:bg-red-50" onClick={() => { onDelete(listing.id); onClose(); }}>
