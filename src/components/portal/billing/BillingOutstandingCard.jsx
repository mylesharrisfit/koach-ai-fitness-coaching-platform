...
function DueBadge({ dueDate }) {
  ...
  if (days < 0) return <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(239,68,68,0.2)', color: 'rgb(var(--destructive))' }}>Overdue {Math.abs(days)}d</span>;
  if (days <= 3) return <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(245,158,11,0.2)', color: 'rgb(var(--warning))' }}>Due in {days}d</span>;
            ...
            {unpaidInvoices.map(inv => (
              <div key={inv.id} className="flex items-center gap-3 py-2 px-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.06)' }}>
                ...
                <button onClick={() => onPayInvoice(inv)}
                  ...
                  style={{ background: 'rgba(37,99,235,0.3)', color: 'rgb(var(--primary))' }}>
                  Pay
                </button>
              </div>
            ))}
