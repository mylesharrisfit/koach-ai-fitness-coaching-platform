    ...
    <div className="flex flex-col items-center gap-1.5">
      <div className="relative" style={{ width: size, height: size }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          {size > 80 && <p className="text-slate-400 text-[10px] mt-0.5">{unit}</p>}
        </div>
      </div>
      <p className="text-slate-500 text-[9px] font-bold text-center">{label}</p>
      <p className="text-slate-400 text-[8px] text-center">{Math.round(remaining)}{unit} left</p>
    </div>
  );
}
    ...
    <div className="flex flex-col items-center">
      <div className="relative" style={{ width: size, height: size }}>
        <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
          ...
          <circle cx={size / 2} cy={size / 2} r={r} fill="none" strokeWidth={stroke} stroke="#FEF3C7" />
          <circle cx={size / 2} cy={size / 2} r={r} fill="none" strokeWidth={stroke}
            stroke={over ? '#EF4444' : 'url(#calGrad2)'}
            strokeDasharray={circ} strokeDashoffset={offset}
            strokeLinecap="round" style={{ transition: 'stroke-dashoffset 0.7s ease' }} />
        </svg>
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <p className="text-slate-900 font-black text-2xl leading-none">{Math.round(consumed)}</p>
          <p className="text-slate-400 text-[10px] mt-0.5">of {target}</p>
          <p className="text-slate-400 text-[9px]">kcal</p>
        </div>
      </div>
...
export default function MacroRings({ logged, plan }) {
  ...
  return (
    <div className="mx-4 bg-white p-5 rounded-3xl" style={{ boxShadow: '0 2px 20px rgba(0,0,0,0.06)', border: '1px solid #F1F5F9' }}>
      <div className="flex items-center gap-6">
        <BigCalorieRing consumed={logged.calories || 0} target={targets.calories} />
        <div className="flex gap-4 flex-1 justify-around">
          <MiniRing value={logged.protein || 0} target={targets.protein} color="#2563EB" trackColor="#DBEAFE" label="Protein" />
          <MiniRing value={logged.carbs || 0} target={targets.carbs} color="#F97316" trackColor="#FFEDD5" label="Carbs" />
          <MiniRing value={logged.fats || 0} target={targets.fats} color="#EAB308" trackColor="#FEF9C3" label="Fats" />
