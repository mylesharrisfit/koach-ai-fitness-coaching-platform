    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ y: 300 }} animate={{ y: 0 }} exit={{ y: 300 }}
        ...
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-slate-900 font-black text-base">Share with Community</h3>
        ...
        <div className="flex items-center gap-3 mb-3">
          <div className="w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm text-white flex-shrink-0"
            style={{ background: anonymous ? '#94A3B8' : 'linear-gradient(135deg, #2563EB, #7C3AED)' }}>
        ...
        <textarea
          ...
          className="w-full p-3 rounded-2xl bg-slate-50 border border-slate-200 text-slate-800 text-sm resize-none focus:outline-none focus:border-blue-300"
          rows={4}
        />
        ...
        <div className="flex items-center justify-between mt-3">
          <div className="flex items-center gap-3">
            <button onClick={() => fileRef.current?.click()}
              className="flex items-center gap-1.5 text-slate-400 text-xs font-semibold">
              <ImageIcon size={16} />
            ...
            <button onClick={() => setAnonymous(!anonymous)}
              className={`flex items-center gap-1.5 text-xs font-semibold transition-colors ${anonymous ? 'text-blue-600' : 'text-slate-400'}`}>
              <EyeOff size={16} />
              Anonymous
            </button>
          </div>
  ...
  if (!guidelinesAccepted) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-6 text-center">
        ...
        <h2 className="text-slate-900 font-black text-xl mb-2">Community Guidelines</h2>
        ...
        <div className="text-left w-full max-w-xs space-y-2 mb-6">
          {['Be supportive and positive 🙌', 'Respect everyone\'s journey 💙', 'No spam or self-promotion 🚫', 'Content is private to members 🔒'].map(g => (
            <div key={g} className="flex items-center gap-2">
              ...
              <p className="text-slate-600 text-sm">{g}</p>
            </div>
          ))}
        </div>
      ...
      <div className="space-y-0 pb-4">
        ...
        {regularPosts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
            ...
            <h3 className="text-slate-900 font-black text-lg mb-2">Welcome to the Community!</h3>
            <p className="text-slate-400 text-sm mb-6">Be the first to post and introduce yourself to the group.</p>
