...
function RiskBadge({ score }) {
  if (score >= 80) return <Badge className="bg-destructive/10 text-destructive border-destructive text-xs">High Risk</Badge>;
  if (score >= 60) return <Badge className="bg-warning/10 text-warning border-warning text-xs">Medium Risk</Badge>;
  return <Badge className="bg-warning/10 text-warning border-warning text-xs">Low Risk</Badge>;
}

function RiskBar({ score }) {
  const color = score >= 80 ? 'bg-destructive' : score >= 60 ? 'bg-warning' : 'bg-warning';
  return (
        ...
        <div>
          ...
          <p className="text-sm text-foreground">
            {clients.length > 0
              ...
              : <span className="text-success font-semibold">All clients are engaged ✓</span>
            }
          </p>
        </div>
        {clients.length === 0 && <CheckCircle2 className="w-6 h-6 text-success" />}
      </div>
                ...
                <tr key={client.id} className="hover:bg-muted transition-colors">
                  <td className="py-3 pr-4">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-accent/10 flex items-center justify-center text-xs font-bold text-primary flex-shrink-0">
                  ...
                  <td className="py-3 pr-4">
                    <div className="flex items-center gap-1.5 text-muted-foreground">
                      ...
                      {client.daysSinceCheckIn !== null
                        ? <span className={cn(client.daysSinceCheckIn > 21 ? 'text-destructive font-medium' : 'text-warning')}>
