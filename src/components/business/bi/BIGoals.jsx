...
function GoalCard({ goal, current, unit, onEdit }) {
  ...
  const color = achieved ? '#22C55E' : pct >= 70 ? '#F59E0B' : '#3B82F6';

  return (
    <div className={`p-3.5 rounded-xl border relative ${achieved ? 'border-green-200 bg-green-50' : 'border-gray-100 bg-gray-50'}`}>
      {achieved && (
      ...
      <div className="flex items-center justify-between mb-2">
        <p className="text-xs font-semibold text-gray-700">{goal.label}</p>
      ...
      <p className="text-[10px] text-gray-400 mt-1.5">{pct}% to goal{achieved ? ' 🎉' : ''}</p>
    </div>
  );
}
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
      ...
      {editing && (
        <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-xl">
          <p className="text-xs font-semibold text-blue-700 mb-2">{GOAL_PRESETS.find(g => g.key === editing)?.label}</p>
          <div className="flex gap-2">
            ...
            <button onClick={saveEdit} className="w-8 h-8 bg-green-500 text-white rounded-lg flex items-center justify-center hover:bg-green-600">
              <Check className="w-4 h-4" />
            </button>
            <button onClick={() => setEditing(null)} className="w-8 h-8 bg-gray-200 text-gray-600 rounded-lg flex items-center justify-center hover:bg-gray-300">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
      ...
      <p className="text-[10px] text-gray-400 text-center mt-3">Click ✏️ on any goal to set your target</p>
    </div>
  );
}