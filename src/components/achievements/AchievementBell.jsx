        ...
        >
          ...
          <div className="max-h-72 overflow-y-auto">
            {recentBadges.length === 0 ? (
              <p className="text-xs text-[#4B5563] text-center py-6">No achievements in last 24h</p>
            ) : recentBadges.map(b => {
              const cfg = BADGE_CONFIG[b.badge_key];
              const tier = cfg ? TIER_STYLES[cfg.tier] : null;
              if (!cfg || !tier) return null;
              return (
                <div key={b.id} className="flex items-center gap-3 px-4 py-3 border-b border-white/5 hover:bg-white/5 transition-colors">
                  ...
                  <p className="text-[9px] text-[#4B5563] flex-shrink-0">
