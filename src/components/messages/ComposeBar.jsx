...
import {
  ...
  Mic, Video, Loader2, RotateCw, Check, Edit3, Link
} from 'lucide-react';
...
function AIReplyPreview({ suggestion, loading, onUse, onEditFirst, onRetry, onDismiss }) {
  return (
    <div className="mx-1 mb-2 rounded-xl border border-primary/20 bg-gradient-to-br from-[#EEF4FF] to-[#F5F3FF] p-3 shadow-sm">
      <div className="flex items-center justify-between mb-2">
        ...
        <button onClick={onDismiss} className="text-[#9CA3AF] hover:text-gray-600 transition-colors">
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={() => onUse(suggestion)} className="flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 bg-primary text-white rounded-full hover:bg-primary/90 transition-colors">
              <Check className="w-3 h-3" /> Use This
            </button>
            <button onClick={() => onEditFirst(suggestion)} className="flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 bg-white border border-primary/30 text-primary rounded-full hover:bg-primary/5 transition-colors">
              <Edit3 className="w-3 h-3" /> Edit First
            </button>
            <button onClick={onRetry} className="flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 bg-white border border-gray-200 text-[#6B7280] rounded-full hover:bg-gray-50 transition-colors">
              <RotateCw className="w-3 h-3" /> Try Again
            </button>
            <button onClick={onDismiss} className="text-[11px] text-[#9CA3AF] hover:text-gray-600 underline ml-auto">Dismiss</button>
          </div>
        </>
      ) : null}
    </div>
...
export default function ComposeBar({ client, allMessages, checkIns = [], onSend, selectedTag, setSelectedTag, value, onChange }) {
  ...
  const generateAIReply = useCallback(async () => {
    ...
    const lastFew = [...clientMessages].sort((a, b) => new Date(a.created_date) - new Date(b.created_date)).slice(-8);
    const convo = lastFew.map(m => `${m.sender === 'coach' ? 'Coach' : client.name}: ${m.content}`).join('\n');
    const lastCI = [...checkIns].sort((a, b) => new Date(b.date) - new Date(a.date))[0];
    const checkInCtx = lastCI ? `\nLast check-in (${lastCI.date}): weight ${lastCI.weight}lbs, training ${lastCI.compliance_training}%, nutrition ${lastCI.compliance_nutrition}%, mood: ${lastCI.mood}` : '';
    const result = await base44.integrations.Core.InvokeLLM({
      prompt: `You are a personal fitness coach. Write ONE short, warm, human reply to your client "${client.name}".\n\nContext:\n- Goal: ${client.goal || 'general fitness'}\n- Program: ${client.assigned_program_id ? 'assigned' : 'not assigned'}${checkInCtx}\n\nRecent conversation:\n${convo || 'No messages yet.'}\n\nInstructions: sound like a real caring coach, under 60 words, use client's first name, vary your opener. Return only the message text.`,
      response_json_schema: { type: 'object', properties: { message: { type: 'string' } } }
    });
    setAiSuggestion(result.message || '');
    setAiLoading(false);
  }, [client, allMessages, checkIns]);
    ...
    <div className="border-t border-[#E7EAF3] bg-white flex-shrink-0">
      {showAI && (
        <div className="px-3 pt-3">
          <AIReplyPreview
            suggestion={aiSuggestion} loading={aiLoading}
            onUse={(text) => { onChange(text); setShowAI(false); textareaRef.current?.focus(); }}
            onEditFirst={(text) => { onChange(text); setShowAI(false); textareaRef.current?.focus(); }}
            onRetry={generateAIReply}
