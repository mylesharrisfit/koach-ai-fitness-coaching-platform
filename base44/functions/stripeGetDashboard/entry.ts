...
Deno.serve(async (req) => {
  try {
    ...
    // Scope payments to THIS coach's clients only — never return other coaches' revenue
    const payments = await base44.asServiceRole.entities.Payment.filter({ created_by_id: user.id }, '-paid_date', 200);
